...
def filter_img_objects(color_image, lower, upper):   
    ...
    # El resultado es la imagen filtrada, el centriode del objeto en coordenadas de la imagen
    ...
    # En estas variables se deben guardar el centroide del objeto en coordenadas de pixeles,
    ...
    if contours:
        # Encuentra el contorno de mayor área para conocer su centroide
        ...
        # bandera para decir que no existió objeto en la máscara
        detection = False
...
def img_xyz(centroid, depth, camera_matrix):
    ...
    px, py = centroid
    z = depth / 1000.0  # Convertir profundidad a metros
    x = (px - cx) * z / fx
    y = (py - cy) * z / fy
...
def color_image_callback(color_image_msg):
    ...
    # Filtrar objeto rojo (límites de la máscara en HSV)
...
def depth_image_callback(depth_image_msg):
    ...
    # Si la variable de objeto o robot detectado es False no se tiene profundidad.
...
def ros_node():
    ...
    # Se debe completar los topics de la imagen de profundidad y de la informacion de la camara.
    
    color_image_sub = rospy.Subscriber('/camera/color/image_raw', Image, color_image_callback)
    depth_image_sub = rospy.Subscriber('/camera/depth/image_raw', Image, depth_image_callback)  # Callback correspondiente: depth_image_callback
    camera_info_sub = rospy.Subscriber('/camera/depth/camera_info', CameraInfo, camera_info_callback)  # Callback correspondiente: camera_info_callback
